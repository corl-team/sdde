...
from .ensemble import LeNet_Ensemble, ResNet18_32x32_Ensemble, \
    ResNet18_32x32_Multihead, ResNet18_224x224_Ensemble, BaseEnsemble, \
    ResNet50_Ensemble, ResNet18_64x64_Ensemble
...
def get_network(network_config):
    ...
    if get_config_default(network_config, 'modification', None) and network_config.modification == 't2fnorm':
    ...
    elif network_config.name == 'resnet18_32x32_ensemble':
        net = ResNet18_32x32_Ensemble(num_classes=num_classes, num_models=num_models, discriminator_cls=discriminator_cls)

    elif network_config.name == 'resnet18_64x64_ensemble':
        net = ResNet18_64x64_Ensemble(num_classes=num_classes, num_models=num_models,
                                      discriminator_cls=discriminator_cls)

    elif network_config.name == 'resnet18_32x32_multihead':
        net = ResNet18_32x32_Multihead(num_classes=num_classes, num_models=num_models, discriminator_cls=discriminator_cls)

    elif network_config.name == 'resnet50_ensemble':
        net = ResNet50_Ensemble(num_classes=num_classes, discriminator_cls=discriminator_cls)

    elif network_config.name == 'lenet_ensemble':
        net = LeNet_Ensemble(num_classes=num_classes, num_models=num_models, discriminator_cls=discriminator_cls)
    ...
    if network_config.pretrained:
        if type(net) is dict:
            ...
            pass
        elif isinstance(net, BaseEnsemble):
            for i in range(net.num_models):
                net.nets[i].load_state_dict(
                    torch.load(network_config.checkpoint), strict=False
                )
        else:
            try:
        ...
        if type(net) is dict:
            for key, subnet in zip(net.keys(), net.values()):
                net[key] = torch.nn.parallel.DistributedDataParallel(
                    nn.SyncBatchNorm.convert_sync_batchnorm(subnet.cuda()),
                    device_ids=[comm.get_local_rank()],
                    broadcast_buffers=True)
        else:
            net = torch.nn.parallel.DistributedDataParallel(
                nn.SyncBatchNorm.convert_sync_batchnorm(net.cuda()),
                device_ids=[comm.get_local_rank()],
                broadcast_buffers=True)

    if torch.cuda.is_available() and (network_config.num_gpus > 0):
        if type(net) is dict:
