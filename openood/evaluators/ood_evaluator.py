    ...
    def _eval_ood(self,
        ...
        for dataset_name, ood_dl in ood_data_loaders[ood_split].items():
            ...
            if self.config.recorder.name == 'wandb':
                self._log_wandb(ood_metrics, dataset_name=dataset_name)
        ...
        if self.config.recorder.name == 'wandb':
            self._log_wandb([float(x) for x in metrics_mean], dataset_name=ood_split)

    def _log_wandb(self, metrics, dataset_name):
        fpr, auroc, aupr_in, aupr_out, accuracy = metrics

        write_content = {
            f'{dataset_name}-FPR@95': 100 * fpr,
            f'{dataset_name}-AUROC': 100 * auroc,
            f'{dataset_name}-AUPR_IN': 100 * aupr_in,
            f'{dataset_name}-AUPR_OUT': 100 * aupr_out,
            f'{dataset_name}-ACC': 100 * accuracy
        }

        wandb.log(write_content)
